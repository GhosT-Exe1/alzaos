...
if($get_contrato == ""){
	...
	$query = $pdo->query("SELECT * FROM temp_texto WHERE texto = '$texto'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	if(@count($res) > 0){
		$id = $res[0]['id'];
	}else{
		//inserir o texto na tabela temporaria
		$query = $pdo->prepare("INSERT INTO temp_texto SET texto = :texto, empresa = '$id_empresa', cabecalho = '$cabecalho_form', cliente = '$cliente', modelo = '$modelo', assinatura = '$nome_img'");
...
<body>
    ...
    <form method="POST" action="contrato_class.php" onsubmit="return prepararAssinatura()">
