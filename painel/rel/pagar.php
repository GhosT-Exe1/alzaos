...
if($mostrar_registros == 'Não'){
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from pagar where vencimento < curDate() and pago != 'Sim' and usuario_lanc = '$id_usuario' and empresa = '$id_empresa' order by id desc");
	}else{
	$query = $pdo->query("SELECT * from pagar where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and usuario_lanc = '$id_usuario' and empresa = '$id_empresa' order by id desc");
	}
}else{
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from pagar where vencimento < curDate() and pago != 'Sim' and empresa = '$id_empresa' order by id desc");
	}else{
	$query = $pdo->query("SELECT * from pagar where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and empresa = '$id_empresa' order by id desc");
	}
}
