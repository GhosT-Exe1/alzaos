...
// debug
file_put_contents('debug_update.txt', print_r([$query->queryString, $nome, $id_empresa], true));
$query->execute();

// Processa a integração com a Nuvem Fiscal
if (isset($_FILES['certificado']) || isset($_POST['senha_certificado']) || isset($_POST['ambiente_nf']) || isset($_POST['regime_tributario']) || isset($_POST['inscricao_municipal']) || isset($_POST['inscricao_estadual']) || isset($_POST['cnae']) || isset($_POST['aliquota_iss'])) {
    $ch = curl_init('processar_nuvem_fiscal.php');
    ...
    if (isset($_FILES['certificado'])) {
        $postData['certificado'] = $_FILES['certificado'];
    }
    ...
    if (is_array($responseData) && ($responseData['status'] ?? '') == 'error') {
        echo $responseData['message'] ?? 'Erro desconhecido';
        exit();
    }
}
